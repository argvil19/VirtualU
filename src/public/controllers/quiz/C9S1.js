app.controller('C9QuizCtrl', ['$scope', 'CurrentUser', function($scope, CurrentUser, $http) {
    ...
    $scope.q1options = [
        { text: "0", hints: "AND is TRUE if both arguments are TRUE.  Non-zero values are interpreted as TRUE", correct: 0, type: "mc", selected : {} },
        { text: "1", hints: "", correct: 1, type: "mc", selected : {} },
        { text: "None of the above", hints: "output of a relational operator is either TRUE or FALSE", correct: 0, type: "mc", selected : {} },
    ]


    $scope.q2options = [
        { text: "10", hints: "Check out chapter 9 tutorials", correct: 1, type: "sa" }, //sa == short answer
    ...
    $scope.q3options = [
        { text: "0", hints: "OR is TRUE if either argument is TRUE.  Non-zero values are interpreted as TRUE", correct: 0, type: "mc" },
        { text: "1", hints: "", correct: 1, type: "mc" },
        { text: "None of the above", hints: "output of a relational operator is either TRUE or FALSE", correct: 0, type: "mc" },

    ]

    $scope.q4options = [
        { text: "10", hints: "Check out chapter 9 tutorials", correct: 1, type: "sa" }
    ]

    $scope.q5options = [
        { text: "0", hints: "Relational operators are all equal precedence and are evaluated left-to-right", correct: 0, type: "mc" },
        { text: "1", hints: "", correct: 1, type: "mc" },
        { text: "None of the above", hints: "output of a relational operator is either TRUE or FALSE", correct: 0, type: "mc" }
    ]
    ...
    $scope.q7options = [
        { text: "1", hints: "Relational operators are all equal precedence and are evaluated left-to-right", correct: 0, type: "mc" },
        { text: "0", hints: "", correct: 1, type: "mc" },
        { text: "None of the above", hints: "output of a relational operator is either TRUE or FALSE", correct: 0, type: "mc" }
    ]
    ...
    $scope.CheckAll = function() {
        ...
        $scope.questions.forEach( function(element, index) {
            // statements
            console.log(element.options[0].selected.correct)
            if(element.options[0].selected.correct)
            {
               mscore += 1
            }
        ...
        $scope.score = (mscore / $scope.questions.length )

        var grades = CurrentUser.currentUser.report

        grades.forEach( function(element, index) {
            if(element.title == 'Chapter 9')
            {
                if(($scope.score * 100) > element.score)
                {
                    element.score = $scope.score * 100
                    element.attempt += 1
                    element.attemptDate = new Date();
                }
                else
                {
                    element.attempt += 1
