    ...
    app.controller('codeCtrlC8S1', ['$scope', 'CurrentUser', function($scope, CurrentUser, $http) {
    $scope.q1hints;
    ...
    $scope.CheckAnswer = function(options) {
        ...
        if($scope.options.selected.type == "mc")
        {
            if($scope.options.selected.correct == 1)
            {
                ...
                console.log(CurrentUser.currentUser.report[0].score)
                CurrentUser.currentUser.report[0].score += 5
            }
            else
            {
